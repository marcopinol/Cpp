...
int j; //externs variables are declared outside of any function, and can be accessed by any other function 
	   //andwill start with value 0 if not initialized
			  
int k = 237; //this variable class is created on compile time, different from auto variables that are created on runtime.
